...
const ProfileMusic = ({ navigation }) => {
  ...
  const getdata = async () => {
    ...
    setid(id !== null ? JSON.parse(id)?.id : "1")
  }
  useEffect(() => {
    dispatch(LoadUser(JSON.parse(Id)))

  }, [dispatch, Id, LoadUser])
  ...
  return (
    <>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.container}>
        <View>
          <Image resizeMode="cover" style={{ width: Dimensions.get("screen").width, height: (Dimensions.get("screen").width) / 1.4 }} source={user?.profile_picture !== null ? { uri: user?.profile_picture } : require("../../../../assets/images/musicss.png")} />
        ...
        <View style={{ margin: 10 }}>
          <ProfileCard name={user?.brand_name} />
        </View>
        <View
          style={{
        ...
        >
          <View>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              />
              <Text style={styles.heading}>{user?.specific_user_rating ? user?.specific_user_rating : 1}</Text>
            </View>
          ...
          <View>
            <Text style={[styles.heading, { textAlign: "center" }]}>{user?.social_media_followers ? user?.social_media_followers : 1}</Text>
        ...
        {/* @Contact View */}
        <View style={{ margin: 10 }}>
          <ProfileContactCard email={user?.email} phone={user?.mobile_number} />
        </View>
        <MyButton
